...
move_tiles_sound = os.path.join(
    root_dir, "../puzzle_simple_game/sounds", "move_tiles.wav")
congratulations = os.path.join(
    root_dir, "../puzzle_simple_game/menu", "congratulations_window.py")
...
rows, cols = (2, 3)  # 2 rows, 3 columns for 6 tiles
...
pygame.display.set_caption("Puzzle Game 2024 [Easy Mode]")
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            # Initialize a list to hold the string representation of each row
            matrix_strings = []
                                    ...
                                    if game_over == False:
                                        if tiles[k].selected:
                                            if tiles[k].movable:
                                                ...
                                                is_game_over()
                                                check_mobility()
                                                moves_count += 1  # Increment moves counter
                                                print(
                                                    "Moves:", moves_count
                                                )  # Print moves count
                                                current_numbers_list = " ".join(
                                                    str(num)
                                                    for row in matrix
                                                    for num in row
                                                )
                                                current_numbers_list = (
                                                    current_numbers_list.replace(
                                                        " ", ""
                                                    )
                                                )
                                                print(
                                                ...
                                                if current_numbers_list == "12345":
                                                    subprocess.Popen(["python", congratulations])

                                                break  # Exit the loop since we found the cell
            if cell_found == False:
                for k in range(tile_count):
                    if tiles and tiles[k].selected:
